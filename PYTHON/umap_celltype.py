...
class scANVIvisualizer:
    def __init__(self, adata_predicted, adata_clustered,use_rep='X_pca',ndim_pca=50, base_output="../Dataset/My_merged_protein_coding_genes"):
        ...
        if self.use_rep=='X_pca': # X_pca
            ...
            self.plot_dir = os.path.join(base_output, "Harmony_scANVI_plot",f'Harmony{self.ndim_pca}')
            self.eval_dir = os.path.join(base_output, "Harmony_scANVI_evaluation",f'Harmony{self.ndim_pca}')
            self.comp_dir = os.path.join(base_output, "Harmony_scANVI_celltype_composition",f'Harmony{self.ndim_pca}')
    ...
    def prepare_umap(self, n_neighbors=20):
        ...
        if self.use_rep=='X_pca':
            ...
            adata_query.obsm["scANVI_harmony"] = adata_query.obsm["X_harmony"]
            sc.pp.neighbors(adata_query, n_neighbors=n_neighbors, use_rep="scANVI_harmony")
    ...
    #----------------------------【UMAP繪圖與儲存】----------------------------
    def plot_umap_and_composition(self, cluster_key="leiden_nn20_res0.6"):
        ...
        sc.pl.umap(
            ...
            palette={
                "Control":"#67D6F0", 
                "NMOSD": "#E3A19F"},
            save=f"_{cluster_key}_group.png"    
        )
        sc_path = f"figures/umap_{cluster_key}_group.png"
        dst_path = f"{sub_dir}/{cluster_key}_UMAP_group.png"
        ...
        sc.pl.umap(
            ...
            title="NMOSD/Control Leiden after annotation",
            legend_loc="right margin",
            legend_fontsize=6,
            frameon=True,
        ...
        sc.pl.umap(
            ...
            save=f"_{cluster_key}_batch.png",
            show=False
            )
        sc_path = f"figures/umap_{cluster_key}_batch.png"
        dst_path = f"{sub_dir}/UMAP_batch.png"
        self.save_with_dpi(sc_path, dst_path, dpi=500)
        ...
        for cond in ["NMOSD", "Control"]:
            sc.pl.umap(
                ...
                color="cluster_annotation",
                title=f"{cond} Leiden after scANVI annotation",
                legend_loc="right margin",
                legend_fontsize=6,
                frameon=True,
                legend_fontoutline=1,
        ...
        plt.title("Proportion of NMOSD/Control cells in each celltype after scANVI annotation", fontsize=22)
    ...
    def evaluate_clustering(self, cluster_key="leiden_nn20_res0.6",label_key="predicted_label"):
        ...
        out_path = f"{self.eval_dir}/{cluster_key}.csv"
    ...
    def export_celltype_composition(self, cluster_key="leiden_nn20_res0.6",label_key="predicted_label"):
        ...
        out_path = f"{self.comp_dir}/{cluster_key}.csv"
